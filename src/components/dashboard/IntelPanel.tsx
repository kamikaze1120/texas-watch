...
const IntelPanel = () => {
  ...
  const generateSitRep = (): string => {
    ...
    let report = `SITUATION REPORT — ${new Date().toLocaleString('en-US', { timeZone: 'America/Chicago', hour12: true })} CST\n\n`;
    ...
    if (total === 0) {
      report += `No dispatch data currently available. Feeds refresh every 15s.`;
    }
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: 'Intel Analyst ready.\n\nClick "Sit Rep" for a real-time situation report based on live dispatch data.' },
  ]);
      ...
      <div className="p-3 border-b border-border/50">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1">
            ...
            <p className="text-[10px] text-muted-foreground font-body tracking-wider">LIVE DATA</p>
          </div>
          <div className="flex items-center gap-1 bg-primary/10 px-2 py-0.5 rounded">
            <Sparkles className="h-2.5 w-2.5 text-primary" />
            <span className="text-[9px] font-display text-primary tracking-wider">AI</span>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {messages.map((msg, i) => (
            <div key={i} className={`${msg.role === 'user' ? 'ml-6' : ''}`}>
              {msg.role === 'assistant' && (
                <span className="text-[9px] font-display text-primary/60 mb-0.5 block tracking-[0.12em]">ANALYST</span>
              )}
              <div className={`text-sm leading-relaxed whitespace-pre-wrap ${
                msg.role === 'user'
                  ? 'bg-primary/8 border border-primary/15 rounded-md p-2.5 text-foreground'
                  : 'text-secondary-foreground bg-secondary/20 rounded-md p-2.5'
              }`}>
                {msg.content}
              </div>
            </div>
      ...
      <div className="p-3 border-t border-border/50">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-secondary/30 border border-border/50 rounded-md px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-1 focus:ring-primary/30 font-body"
          />
          <button
            onClick={handleSend}
        ...
        <div className="flex gap-1.5 mt-2 flex-wrap">
          {['Sit Rep', 'Critical calls', 'Weather alerts'].map(q => (
            <button
              ...
              className="text-xs font-display px-2.5 py-1 rounded border border-border/40 text-muted-foreground/70 hover:text-primary hover:border-primary/20 hover:bg-primary/5 transition-all tracking-wider"
            >
              {q}
            </button>
          ))}
